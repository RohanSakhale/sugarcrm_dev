...
class SugarBean
{
     ...
     * All implementing classes  must call this constructor using the parent::__construct() class.
     *
     */
    public function __construct()
    {
        ...
        if ($this->bean_implements('ACL') && !empty($GLOBALS['current_user'])) {
            $this->acl_fields = (isset($dictionary[$this->object_name]['acl_fields']) && false === $dictionary[$this->object_name]['acl_fields']) ? false : true;
        }
    ...
    public function unPopulateDefaultValues()
    {
        ...
        foreach ($this->field_defs as $field => $value) {
            ...
            if (!empty($this->$field) && !empty($value['display_default']) && in_array($value['type'], ['date', 'datetime', 'datetimecombo']) && $this->parseDateDefault($value['display_default'], ('date' != $value['type'])) == $this->$field) {
                $this->$field = null;
            }
        }
    }
     ...
     * Internal function, do not override.
     */
    public function removeRelationshipMeta($key, $db, $tablename, $dictionary, $module_dir)
    {
    ...
    public function load_relationship($rel_name)
    {
        ...
        if (!empty($fieldDefs[$rel_name])) {
            ...
            if (isset($fieldDefs[$rel_name]['type']) && 'link' == $fieldDefs[$rel_name]['type']) {
                if ("Link2" == $class) {
     ...
     * Internal function, do not override.
     */
    public function get_linked_beans($field_name, $bean_name, $sort_array = [], $begin_index = 0, $end_index = -1,
        ...
        if ($this->load_relationship($field_name)) {
            if ($this->$field_name instanceof Link) {
                ...
                // Link2 style
                if (-1 != $end_index || !empty($deleted) || !empty($optional_where)) {
        ...
        if (!empty($fieldDefs)) {
            foreach ($fieldDefs as $key => $value_array) {
                if ((isset($value_array['importable'])
                    && (is_string($value_array['importable']) && 'false' == $value_array['importable'] || is_bool($value_array['importable']) && false == $value_array['importable']))
                    || (isset($value_array['type']) && 'link' == $value_array['type'])
                    || (isset($value_array['auto_increment'])
                        && (true == $value_array['type'] || 'true' == $value_array['type']))) {
                    // only allow import if we force it
                    if (isset($value_array['importable'])
                        && (is_string($value_array['importable']) && 'true' == $value_array['importable'] || is_bool($value_array['importable']) && true == $value_array['importable'])) {
                    ...
                    if (isset($value_array['type']) && ('relate' == $value_array['type']) && isset($value_array['id_name'])) {
                        $idField = $value_array['id_name'];
                        if (isset($fieldDefs[$idField]) && isset($fieldDefs[$idField]['type']) && 'link' == $fieldDefs[$idField]['type']) {
...
//        require_once('data/Link.php');
    ...
    public function get_import_required_fields()
    {
        ...
        foreach ($importable_fields as $name => $properties) {
            if (isset($properties['importable']) && is_string($properties['importable']) && 'required' == $properties['importable']) {
    ...
    public function cleanBean()
    {
        foreach ($this->field_defs as $key => $def) {
            ...
            if ('html' == $def['type'] || 'longhtml' == $def['type']) {
                $this->$key = SugarCleaner::cleanHtml($this->$key, true);
            } elseif ((strpos($type, 'char') !== false ||
                strpos($type, 'text') !== false || 'enum' == $type) &&
                !empty($this->$key)
            ) {
    ...
    public function send_assignment_notifications($notify_user, $admin)
    {
        ...
        if (('Meeting' == $this->object_name || 'Call' == $this->object_name) || $notify_user->receive_notifications) {
    ...
    public function create_notification_email($notify_user)
    {
        ...
        $port = (80 != $parsedSiteUrl['port']) ? ":" . $parsedSiteUrl['port'] : '';
    ...
    protected function set_relationship_info($exclude = [])
    {
        ...
        $new_rel_link = false;
        // check incoming data
        if (isset($this->not_use_rel_in_req) && true == $this->not_use_rel_in_req) {
        ...
        if ($rel_id && $rel_link && !in_array($rel_link, $exclude) && $rel_id != $this->id) {
            ...
            if ($isRelationshipLoaded && !empty($this->$rel_link) && $this->$rel_link->getRelationshipObject() && $this->$rel_link->getRelationshipObject()->getLHSModule() == $this->$rel_link->getRelationshipObject()->getRHSModule()) {
                ...
                foreach ($this->field_defs as $key => $def) {
                    if (isset($def['type']) && 'link' == $def['type'] && isset($def['relationship']) && $def['relationship'] == $rel_link) {
                        $new_rel_link = $key;
                    }
                }
            }
        ...
        if (isset($this->relationship_fields) && is_array($this->relationship_fields)) {
            foreach ($this->relationship_fields as $id => $rel_name) {
                ...
                if (!empty($this->$id)) {
                    ...
                    if ($this->$id == $new_rel_id && true == $rel_add) {
                        $new_rel_id = false;
                    }
                } else {
    ...
    protected function handle_remaining_relate_fields($exclude = [])
    {
        ...
        foreach ($this->field_defs as $def) {
            if ('relate' == $def['type'] && isset($def['id_name']) && isset($def['link']) && isset($def['save'])) {
    ...
    {
        foreach ($this->field_defs as $def) {
            if (!empty($def['type']) && "parent" == $def['type']) {
                ...
                if (
                    ...
                    foreach ($this->field_defs as $ldef) {
                        if (!empty($ldef['type']) && "link" == $ldef['type'] && !empty($ldef['relationship'])) {
                ...
                if ($this->load_relationship($lower_link)) {
                    ...
                    if (!empty($rel)) {
                        foreach ($this->field_defs as $field => $def) {
                            if ('link' == $def['type'] && !empty($def['relationship']) && $def['relationship'] == $rel) {
    ...
    public function check_date_relationships_load()
    {
        ...
        foreach ($this->field_defs as $fieldDef) {
            ...
            if (!isset($this->processed_dates_times[$field])) {
                ...
                if ('date_modified' == $field || 'date_entered' == $field) {
                    ...
                    if ('relate' == $type && isset($this->field_name_map[$field]['custom_module'])) {
                        $type = $this->field_name_map[$field]['type'];
                    }

                    if ('date' == $type) {
                        if ('0000-00-00' == $this->$field) {
                            $this->$field = '';
                    ...
                    } elseif ('datetime' == $type || 'datetimecombo' == $type) {
                        if ('0000-00-00 00:00:00' == $this->$field) {
                            $this->$field = '';
                        } else {
                            if (empty($disable_date_format)) {
                    ...
                    } elseif ('encrypt' == $type && empty($disable_date_format)) {
    ...
    public function fixUpFormatting()
    {
        ...
        foreach ($this->field_defs as $field => $def) {
            ...
            if ((isset($def['source']) && 'non-db' == $def['source']) || 'deleted' == $field) {
                continue;
            }
    ...
    public function retrieve($id = -1, $encode = true, $deleted = true)
    {
        ...
        if ($this->optimistic_lock && $module == $this->module_dir && 'EditView' == $action) {
...
// save related fields values for audit
    ...
    public function populateFromRow($row)
    {
        ...
        foreach ($this->field_defs as $field => $field_value) {
            if ('user_preferences' == $field && 'Users' == $this->module_dir) {
                continue;
            }
    ...
    public function process_order_by($order_by, $submodule = null, $suppress_table_name = false)
    {
        ...
        foreach ($raw_elements as $key => $value) {
            ...
            if (isset($bean_queried->field_defs[$list_column_name])) {
                ...
                if ('non-db' != $source && $this->db->isTextType($this->db->getFieldType($bean_queried->field_defs[$list_column_name]))) {
    ...
    protected static function build_sub_queries_for_union($subpanel_list, $subpanel_def, $parentbean, $order_by)
    {
        ...
        foreach ($subpanel_list as $this_subpanel) {
            if (!$this_subpanel->isDatasourceFunction() || ($this_subpanel->isDatasourceFunction()
                ...
                && true == $this_subpanel->_instance_properties['generate_select'])) {
                ...
                foreach ($list_fields as $list_key => $list_field) {
                    if (isset($list_field['usage']) && 'display_only' == $list_field['usage']) {
                        unset($list_fields[$list_key]);
                    }
                }

                if (!$subpanel_def->isCollection() && isset($list_fields[$order_by]) && isset($submodule->field_defs[$order_by]) && (!isset($submodule->field_defs[$order_by]['source']) || 'db' == $submodule->field_defs[$order_by]['source'])) {
     ...
     */
    public function get_union_related_list($parentbean, $order_by = "", $sort_order = '', $where = "",
        ...
        if (empty($final_query)) {
            ...
            foreach ($subqueries as $subquery) {
                ...
                } elseif (false !== $select_position && false != $distinct_position) {
    ...
    public function create_new_list_query($order_by, $where, $filter = [], $params = [], $show_deleted = 0, $join_type = '', $return_array = false, $parentbean = null, $singleSelect = false, $ifListForExport = false)
    {
        ...
        foreach ($fields as $field => $value) {
            ...
            if ((!isset($data['source']) || 'db' == $data['source']) && (!empty($alias) || !empty($filter))) {
                $ret_array['select'] .= ", $this->table_name.$field $alias";
                $selectedFields["$this->table_name.$field"] = true;
            } else if ((!isset($data['source']) || 'custom_fields' == $data['source']) && (!empty($alias) || !empty($filter))) {
            ...
            if ('relate' != $data['type'] && isset($data['db_concat_fields'])) {
            ...
            if ('relate' == $data['type'] && (isset($data['custom_module']) || isset($data['ext2']))) {
            ...
            if ($this->is_relate_field($field)) {
                ...
                if (!empty($this->$data['link'])) {
                    ...
                    $table_joined = !empty($joined_tables[$params['join_table_alias']]) || (!empty($joined_tables[$params['join_table_link_alias']]) && isset($data['link_type']) && 'relationship_info' == $data['link_type']);
...
//27416, the $ret_array['secondary_select'] should always generate, regardless the dbtype
                        ...
                        if (!$table_joined) {
                            ...
                            if (isset($data['link_type']) && 'relationship_info' == $data['link_type'] && ($parentbean instanceof SugarBean)) {
            ...
            foreach ($children_info as $child_info) {
                if ('parent' == $child_info['type']) {
                    if (empty($templates[$child_info['parent_type']])) {
                        ...
                        if ('test' == $child_info['parent_type']) {
                            continue;
                        }
    ...
    public function process_list_query($query, $row_offset, $limit = -1, $max_per_page = -1, $where = '')
    {
        ...
        if (empty($sugar_config['disable_count_query']) || $toEnd) {
            ...
            if (!empty($count_query) && (empty($limit) || -1 == $limit)) {
        ...
        if (!empty($limit) && -1 != $limit && -99 != $limit) {
        ...
        $index = $row_offset;
        while (-99 == $max_per_page || ($index < $row_offset + $max_per_page)) {
    ...
    public function process_union_list_query($parent_bean, $query,
        $row_offset, $limit = -1, $max_per_page = -1, $where = '', $subpanel_def, $query_row_count = '', $secondary_queries = []) {
        $db = DBManagerFactory::getInstance('listviews');
        /**
         * if the row_offset is set to 'end' go to the end of the list
         */
        ...
        if (empty($sugar_config['disable_count_query']) || $toEnd) {
            ...
            if (!empty($rows_found) && (empty($limit) || -1 == $limit)) {
        ...
        if ($performSecondQuery) {
            ...
            while ($row) {
                $function_fields = [];
                if (($index < $row_offset + $max_per_page || -99 == $max_per_page)) {
                    ...
                    foreach ($current_bean->field_defs as $field => $value) {
                        ...
                        if (isset($value['source']) && 'function' == $value['source']) {
                    ...
                    foreach ($function_fields as $function_field) {
                        ...
                        foreach ($value['function_params'] as $param) {
                            if (empty($value['function_params_source']) or 'parent' == $value['function_params_source']) {
                                if (empty($this->$param)) {
                                    $can_execute = false;
                                } else if ('$this' == $param) {
                                    $execute_params[] = $this;
                            ...
                            } else if ('this' == $value['function_params_source']) {
 ...
 */
    ...
    public function fill_in_link_field($linkFieldName, $def)
    {
        ...
        if (!empty($this->field_defs[$linkFieldName]['type']) && "id" == $this->field_defs[$linkFieldName]['type'] && !empty($def['link'])) {
    ...
    {
        if (isset(self::$fileFields[$this->module_name]) && false == $resetCache) {
    ...
    public function build_related_list($query, &$template, $row_offset = 0, $limit = -1)
    {
        ...
        if (!empty($row_offset) && 0 != $row_offset && !empty($limit) && -1 != $limit) {
    ...
    public function get_list_view_array()
    {
        ...
        foreach ($this->field_defs as $field => $value) {
            if (isset($this->$field)) {
                ...
                if (isset($value['hidden']) && true === $value['hidden']) {
                ...
                else if (((!empty($value['type']) && ('enum' == $value['type'] || 'radioenum' == $value['type']))) && empty($value['function'])) {
                ...
                } elseif (!empty($value['custom_module']) && 'currency' != $value['type']) {
//                    $this->format_field($value);
    ...
    public function getRelatedFields($module, $id, $fields, $return_array = false)
    {
        ...
        foreach ($fields as $field => $alias) {
            if (!empty($GLOBALS['dictionary'][$object]['fields'][$field]['db_concat_fields'])) {
                ...
                (empty($GLOBALS['dictionary'][$object]['fields'][$field]['source']) || "non-db" != $GLOBALS['dictionary'][$object]['fields'][$field]['source'])) {
    ...
    public function assign_display_fields($currentModule)
    {
        ...
        foreach ($this->column_fields as $field) {
            if (isset($this->field_name_map[$field]) && empty($this->$field)) {
                if ('date' != $this->field_name_map[$field]['type'] && 'enum' != $this->field_name_map[$field]['type']) {
                    $this->$field = $field;
                }

                if ('date' == $this->field_name_map[$field]['type']) {
                    $this->$field = $timedate->to_display_date('1980-07-09');
                }
                if ('enum' == $this->field_name_map[$field]['type']) {
    ...
    {
        if (!isset($this->processed) || false == $this->processed) {
    ...
    public function toArray($dbOnly = false, $stringOnly = false, $upperKeys = false)
    {
        ...
        foreach ($this->field_defs as $field => $data) {
            if (!$dbOnly || !isset($data['source']) || 'db' == $data['source']) {
 ...
 */
    ...
    private function _checkOptimisticLocking($action, $isUpdate)
    {
        if ($this->optimistic_lock && !isset($_SESSION['o_lock_fs'])) {
            if (isset($_SESSION['o_lock_id']) && $_SESSION['o_lock_id'] == $this->id && $_SESSION['o_lock_on'] == $this->object_name) {
                if ('Save' == $action && $isUpdate && isset($this->modified_user_id) && $this->has_been_modified_since($_SESSION['o_lock_dm'], $this->modified_user_id)) {
    ...
    {
        if ($check_notify || (isset($this->notify_inworkflow) && true == $this->notify_inworkflow) // cn: bug 5795 - no invites sent to Contacts, and also bug 25995, in workflow, it will set the notify_on_save=true.
             && !$this->isOwner($this->created_by)) // cn: bug 42727 no need to send email to owner (within workflow)
        {
            ...
            } elseif (isset($_REQUEST['send_invites']) && 1 == $_REQUEST['send_invites']) {
    ...
    private function _loadCachedArray(
        ...
        if (!empty($result)) {
            ...
            if (SugarCache::EXTERNAL_CACHE_NULL_VALUE == $result) {
                return null;
            }
    ...
    protected function is_relate_field($field)
    {
        ...
        && 'relate' == $field_def['type'] && isset($field_def['link']);
    }
    ...
    {
        if (property_exists($this, 'currency_id') && -99 == $this->currency_id) {
            ...
            if ($currency) {
                ...
                foreach ($this->field_defs as $this_field) {
                    if (isset($this_field['type']) && 'relate' == $this_field['type'] && isset($this_field['module']) && 'Currencies' == $this_field['module'] && isset($this_field['id_name']) && 'currency_id' == $this_field['id_name']) {
    ...
    {
        if (!empty($this->field_defs['email_addresses']) && 'link' == $this->field_defs['email_addresses']['type'] &&
            !empty($this->field_defs['email_addresses_non_primary']) && 'email' == $this->field_defs['email_addresses_non_primary']['type']) {
            return true;
        } else {
            return false;
        }
