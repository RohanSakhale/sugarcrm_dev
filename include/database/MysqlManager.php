...
class MysqlManager extends DBManager
{
    ...
    public function get_columns($tablename)
    {
        ...
        while (($row = $this->fetchByAssoc($result)) != null) {
            ...
            if ('NO' == $row['Null'] && !stristr($row['Key'], 'PRI')) {
    ...
    protected function isEngineEnabled($engine)
    {
        ...
        if (strtoupper($row['Engine']) == $engine) {
            return ('YES' == $row['Support'] || 'DEFAULT' == $row['Support']);
        }
    ...
    protected function oneColumnSQLRep($fieldDef, $ignoreRequired = false, $table = '', $return_as_array = false)
    {
        ...
        if ('int' == $ref['colType'] && !empty($fieldDef['len'])) {
    ...
    public function massageFieldDef(&$fieldDef, $tablename)
    {
        ...
        if (isset($fieldDef['default']) &&
            ('text' == $fieldDef['dbType'] || 'blob' == $fieldDef['dbType'] || 'longtext' == $fieldDef['dbType'] || 'longblob' == $fieldDef['dbType'])) {
            unset($fieldDef['default']);
        }

        if ('uint' == $fieldDef['dbType']) {
            $fieldDef['len'] = '10';
        }

        if ('ulong' == $fieldDef['dbType']) {
            $fieldDef['len'] = '20';
        }

        if ('bool' == $fieldDef['dbType']) {
            $fieldDef['type'] = 'tinyint';
        }

        if ('bool' == $fieldDef['dbType'] && empty($fieldDef['default'])) {
            $fieldDef['default'] = '0';
        }

        if (('varchar' == $fieldDef['dbType'] || 'enum' == $fieldDef['dbType']) && empty($fieldDef['len'])) {
            $fieldDef['len'] = '255';
        }
        ...
        if ('int' == $fieldDef['dbType'] && empty($fieldDef['len'])) {
            $fieldDef['len'] = '11';
        }

        if ('decimal' == $fieldDef['dbType']) {
