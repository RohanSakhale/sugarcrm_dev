...
EOQ;
    ...
    public function addAlert($type, $name, $subtitle, $description, $countdown, $redirect = '')
    {
        $this->script .= 'addAlert(' . json_encode($type) . ',' . json_encode($name) . ',' . json_encode($subtitle) . ',' . json_encode(str_replace(["\r", "\n"], ['', '<br>'], $description)) . ',' . $countdown . ',' . json_encode($redirect) . ')' . "\n";
    }
