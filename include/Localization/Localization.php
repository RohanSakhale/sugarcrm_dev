...
class Localization
{
    ...
    public $currencies = []; // array loaded with current currencies
    ...
    public function getPrecedentPreference($prefName, $user = null, $sugarConfigPrefName = '')
    {
        ...
        if ('default_email_charset' == $prefName) {
            if (null != $user) {
        ...
        $pref = (!isset($sugar_config[$prefName]) || (empty($sugar_config[$prefName]) && '0' !== $sugar_config[$prefName])) ? $pref : $sugar_config[$prefName];
        $pref = (empty($userPref) && '0' !== $userPref) ? $pref : $userPref;
        return $pref;
    }
    ...
    public function getLocaleFormattedNumber($number, $currencySymbol = '', $is_currency = true, $user = null)
    {
        ...
        // handle decimals
        if ($precision > 0) {
            // we toss the minor digits otherwise
...
eoq;
    ...
    public function getLocaleFormattedName($firstName, $lastName, $salutationKey = '', $title = '', $format = "", $user = null, $returnEmptyStringIfEmpty = false)
    {
        ...
        $names = [];
        $names['f'] = (empty($firstName) && 0 != $firstName) ? '' : $firstName;
        $names['l'] = (empty($lastName) && 0 != $lastName) ? '' : $lastName;
        $names['s'] = (empty($salutation) && 0 != $salutation) ? '' : $salutation;
        $names['t'] = (empty($title) && 0 != $title) ? '' : $title;
        ...
        if (strpos($formattedName, ',', strlen($formattedName) - 1)) {
