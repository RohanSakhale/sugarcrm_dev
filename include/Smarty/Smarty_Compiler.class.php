...
class Smarty_Compiler extends Smarty
{
    ...
    public function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        if ($this->security) {
            ...
            if (SMARTY_PHP_ALLOW == $this->php_handling &&
        ...
        for ($curr_tb = 0, $for_max = count($text_blocks); $curr_tb < $for_max; $curr_tb++) {
            ...
            if (preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $text_blocks[$curr_tb], $sp_match)) {
                ...
                /* process each one */
                for ($curr_sp = 0, $for_max2 = count($sp_match[1]); $curr_sp < $for_max2; $curr_sp++) {
                    if (SMARTY_PHP_PASSTHRU == $this->php_handling) {
                        /* echo php contents */
                        $text_blocks[$curr_tb] = str_replace('%%%SMARTYSP' . $curr_sp . '%%%', '<?php echo \'' . str_replace("'", "\'", $sp_match[1][$curr_sp]) . '\'; ?>' . "\n", $text_blocks[$curr_tb]);
                    } else if (SMARTY_PHP_QUOTE == $this->php_handling) {
                        /* quote php tags */
                        $text_blocks[$curr_tb] = str_replace('%%%SMARTYSP' . $curr_sp . '%%%', htmlspecialchars($sp_match[1][$curr_sp]), $text_blocks[$curr_tb]);
                    } else if (SMARTY_PHP_REMOVE == $this->php_handling) {
                        /* remove php tags */
    ...
    public function _compile_tag($template_tag)
    {
        ...
        switch ($tag_command) {
            ...
            case 'else':
                ...
                if ('if' != $_open_tag && 'elseif' != $_open_tag) {
    ...
    public function _compile_include_tag($tag_args)
    {
        ...
        foreach ($attrs as $arg_name => $arg_value) {
            if ('file' == $arg_name) {
                ...
                continue;
            } else if ('assign' == $arg_name) {
                $assign_var = $arg_value;
                continue;
            } else if ('theme_template' == $arg_name) {
        ...
        if ('true' == $theme_template) {
            $include_file = '"' . SugarThemeRegistry::current()->getTemplate(str_replace(['"', "'"], '', $include_file)) . '"';
        }
    ...
    public function _compile_include_php_tag($tag_args)
    {
        ...
        $once_var = (empty($attrs['once']) || 'false' == $attrs['once']) ? 'false' : 'true';

        $arg_list = [];
        foreach ($attrs as $arg_name => $arg_value) {
            if ('file' != $arg_name and 'once' != $arg_name and 'assign' != $arg_name) {
                if (is_bool($arg_value)) {
                    $arg_value = $arg_value ? 'true' : 'false';
                }
    ...
    public function _parse_is_expr($is_arg, $tokens)
    {
        ...
        switch ($expr_type) {
            case 'even':
                if (isset($tokens[$expr_end]) && 'by' == $tokens[$expr_end]) {
                    $expr_end++;
    ...
    public function _pop_tag($close_tag)
    {
        ...
        if (count($this->_tag_stack) > 0) {
            ...
            if ('if' == $close_tag && ('else' == $_open_tag || 'elseif' == $_open_tag)) {
                return $this->_pop_tag($close_tag);
            }
            if ('section' == $close_tag && 'sectionelse' == $_open_tag) {
                $this->_pop_tag($close_tag);
                return $_open_tag;
            }
            if ('foreach' == $close_tag && 'foreachelse' == $_open_tag) {
                $this->_pop_tag($close_tag);
                return $_open_tag;
            }
            if ('else' == $_open_tag || 'elseif' == $_open_tag) {
                $_open_tag = 'if';
            } elseif ('sectionelse' == $_open_tag) {
                $_open_tag = 'section';
            } elseif ('foreachelse' == $_open_tag) {
                $_open_tag = 'foreach';
            }
