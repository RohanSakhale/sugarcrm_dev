...
class SugarEmailAddress extends SugarBean
{
    ...
    public function handleLegacySave($bean, $prefix = "")
    {
        if (!isset($_REQUEST) || !isset($_REQUEST['useEmailWidget'])) {
            if (empty($this->addresses) || !isset($_REQUEST['massupdate'])) {
                $this->addresses = [];
                $optOut = (isset($bean->email_opt_out) && "1" == $bean->email_opt_out) ? true : false;
                $invalid = (isset($bean->invalid_email) && "1" == $bean->invalid_email) ? true : false;
    ...
    public function populateLegacyFields(&$bean)
    {
        ...
        foreach ($this->addresses as $k => $address) {
            ...
            if (1 == $address['primary_address'] && !$primary_found) {
                $primary_index = $k;
                $primary_found = true;
     ...
     */
    public function save($id, $module, $new_addrs = [], $primary = '', $replyTo = '', $invalid = '', $optOut = '', $in_workflow = false)
    {
        ...
        $isConversion = (isset($_REQUEST) && isset($_REQUEST['action']) && 'ConvertLead' == $_REQUEST['action']) ? true : false;
    ...
    public function populateAddresses($id, $module, $new_addrs = [], $primary = '', $replyTo = '', $invalid = '', $optOut = '')
    {
        ...
        $module = (isset($_REQUEST) && isset($_REQUEST['action']) && 'ConvertLead' == $_REQUEST['action']) ? 'Leads' : $module;
                ...
                if ($fromRequest && empty($new_addrs)) {
                    foreach ($_REQUEST as $k => $v) {
                        if (preg_match('/' . $eId . 'emailAddressVerifiedValue[0-9]+$/i', $k) && !empty($v)) {
                            ...
                            if (isset($_REQUEST[$validateFlag]) && "true" == $_REQUEST[$validateFlag]) {
                                $new_addrs[$k] = $v;
                            }
                        }
                    }
    ...
    public function addAddress($addr, $primary = false, $replyTo = false, $invalid = false, $optOut = false, $email_id = null)
    {
        ...
        if (preg_match($this->regex, $addr)) {
            ...
            foreach ($this->addresses as $k => $address) {
                ...
                } elseif ($primary && '1' == $address['primary_address']) {
    ...
    public function getEmailAddressWidgetEditView($id, $module, $asMetadata = false, $tpl = '', $tabindex = '0')
    {
        ...
        $module = $this->getCorrectedModule($module);
        $saveModule = $module;
        if (isset($_POST['is_converted']) && true == $_POST['is_converted']) {
            $id = $_POST['return_id'];
        ...
        if (!empty($id)) {
            ...
            if (empty($prefillDataArr) && "Contacts" == $module) {
        ...
        $this->smarty->assign('addDefaultAddress', (isset($_REQUEST['module']) && 'Emails' == $_REQUEST['module']) ? 'false' : 'true');
        $form = $this->view;

        //determine if this should be a quickcreate form, or a quick create form under subpanels
        if ("QuickCreate" == $this->view) {
            $form = 'form_DC' . $this->view . '_' . $module;
            if (isset($_REQUEST['action']) && 'SubpanelCreates' == $_REQUEST['action'] || 'SubpanelEdits' == $_REQUEST['action']) {
    ...
    public function getEmailAddressWidgetDetailView($focus, $tpl = '')
    {
        ...
        foreach ($prefillData as $addressItem) {
            $key = (1 == $addressItem['primary_address']) ? 'primary' : "";
            $key = (1 == $addressItem['reply_to_address']) ? 'reply_to' : $key;
            $key = (1 == $addressItem['opt_out']) ? 'opt_out' : $key;
            $key = (1 == $addressItem['invalid_email']) ? 'invalid' : $key;
            $key = (1 == $addressItem['opt_out']) && (1 == $addressItem['invalid_email']) ? 'opt_out_invalid' : $key;
 ...
 */
    ...
    {
        return ("Employees" == $module) ? "Users" : $module;
    }
...
function getEmailAddressWidget($focus, $field, $value, $view, $tabindex = '0')
{
    ...
    if ('EditView' == $view || 'QuickCreate' == $view || 'ConvertLead' == $view) {
        $module = $focus->module_dir;
        if ('ConvertLead' == $view && "Contacts" == $module) {
            $module = "Leads";
        }
