...
class UploadFile
{
    ...
    public function confirm_upload()
    {
        ...
        if (UPLOAD_ERR_OK != $_FILES[$this->field_name]['error']) {
            if (UPLOAD_ERR_NO_FILE != $_FILES[$this->field_name]['error']) {
                if (UPLOAD_ERR_INI_SIZE == $_FILES[$this->field_name]['error']) {
                    ...
                    $errMess = string_format($GLOBALS['app_strings']['UPLOAD_ERROR_TEXT_SIZEINFO'], [$_FILES['filename_file']['error'], self::$filesError[$_FILES['filename_file']['error']], $sugar_config['upload_maxsize']]);
    ...
    public function getMime($_FILES_element)
    {
        ...
        $recheckMime = $is_image || (empty($file_ext) && 'application/octet-stream' == $filetype);
    ...
    {
        if (!empty($doc_type) && 'Sugar' != $doc_type) {
            global $sugar_config;
            ...
            try {
                ...
                if (isset($this->api) && false !== $this->api) {
...
class UploadStream
{
    ...
    public static function getSuhosinStatus()
    {
        ...
        if (true == $configuration['suhosin.simulation']) {
            return true;
        }
        ...
        if ('' != $streams) {
            ...
            foreach ($streams as $stream) {
                ...
                if (count($stream) == 1) {
                    if (UploadStream::STREAM_NAME == $stream[0]) {
                        return true;
                    }
                } elseif ('' == $stream[1] && UploadStream::STREAM_NAME == $stream[0]) {
                    return true;
                }
            }
