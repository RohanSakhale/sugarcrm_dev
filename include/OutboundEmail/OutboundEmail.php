...
class OutboundEmail
{
    ...
    public function doesUserOverrideAccountRequireCredentials($user_id)
    {
        ...
        if (0 == $ob->mail_smtpauth_req || $this->isAllowUserAccessToSystemDefaultOutbound() || 'sendmail' == $this->mail_sendtype) {
            return $userCredentialsReq;
        }

        $userOverideAccount = $this->getUsersMailerForSystemOverride($user_id);
        if (null == $userOverideAccount || empty($userOverideAccount->mail_smtpuser) || empty($userOverideAccount->mail_smtpuser)) {
    ...
    public function isAllowUserAccessToSystemDefaultOutbound()
    {
        ...
        if (!empty($a)) {
            ...
            if (isset($admin->settings['notify_allow_default_outbound'])
                && 2 == $admin->settings['notify_allow_default_outbound']) {
                $allowAccess = true;
            }
        }
    ...
    protected function getValues(&$keys)
    {
        ...
        foreach ($keys as $def) {
            if ('mail_smtppass' == $def && !empty($this->$def)) {
                $this->$def = blowfishEncode(blowfishGetKey('OutBoundEmail'), $this->$def);
            } // if
            if ('mail_smtpauth_req' == $def || 'mail_smtpssl' == $def || 'mail_smtpport' == $def) {
                if (empty($this->$def)) {
                    $this->$def = 0;
                }
    ...
    {
        require_once 'include/utils/encryption_utils.php';
        $updateFields = ['mail_smtptype', 'mail_sendtype', 'mail_smtpserver', 'mail_smtpport', 'mail_smtpauth_req', 'mail_smtpssl'];
    ...
    {
        if ("system" == $name && !$this->isAllowUserAccessToSystemDefaultOutbound()) {
