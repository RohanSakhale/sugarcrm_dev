...
class Controller extends SugarBean
{
    ...
    public function change_component_order($magnitude, $direction, $parent_id = "")
    {
        ...
        if (!empty($this->type) && "Save" == $this->type) {
            //safety check
            ...
            if ("Left" == $direction) {
                if ("Y" == $this->focus->controller_def['list_x']) {
                ...
                if ("Y" == $this->focus->controller_def['list_y']) {
            ...
            if (!is_null($row['max_start'])) {
                if ("x" == $this->focus->controller_def['start_axis']) {
                ...
                if ("y" == $this->focus->controller_def['start_axis']) {
...
//If up or left, then simply check the 0 value
        if ("Up" == $direction || "Left" == $direction) {
            if ("x" == $this->focus->controller_def['start_axis']) {
                if (0 == $this->focus->list_order_x) {
                    return false;
                }
            }
            if ("y" == $this->focus->controller_def['start_axis']) {
                if (0 == $this->focus->list_order_y) {
                    return false;
                }
            }
