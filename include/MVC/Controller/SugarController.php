...
class SugarController
{
    ...
    public function setup($module = '')
    {
        ...
        if (!empty($_REQUEST['target_module']) && 'undefined' != $_REQUEST['target_module']) {
    ...
    private function processView()
    {
        ...
        if (!empty($this->bean) && !$this->bean->ACLAccess($view->type) && 'list' != $view->type) {
    ...
    {
        if (!empty($_REQUEST['massupdate']) && 'true' == $_REQUEST['massupdate'] && (!empty($_REQUEST['uid']) || !empty($_REQUEST['entire']))) {
            if (!empty($_REQUEST['Delete']) && 'true' == $_REQUEST['Delete'] && !$this->bean->ACLAccess('delete')
                || (empty($_REQUEST['Delete']) || 'true' != $_REQUEST['Delete']) && !$this->bean->ACLAccess('save')) {
                ACLController::displayNoAccess(true);
                sugar_cleanup(true);
            }
            ...
            $temp_req = ['current_query_by_page' => $_REQUEST['current_query_by_page'], 'return_module' => $_REQUEST['return_module'], 'return_action' => $_REQUEST['return_action']];
            if ('Emails' == $_REQUEST['return_module']) {
            ...
            $_REQUEST = ['return_module' => $temp_req['return_module'], 'return_action' => $temp_req['return_action']]; //for post_massupdate, to go back to original page.
        } else {
 ...
 */
    ...
    protected function action_configuredashlet()
    {
        ...
        if (!empty($_REQUEST['id'])) {
            ...
            if (!empty($_REQUEST['configure']) && $_REQUEST['configure']) {
                ...
                // display options
                $json = getJSONobj();
                return 'result = ' . $json->encode((['header' => $dashlet->title . ' : ' . $mod_strings['LBL_OPTIONS'],
    ...
    protected function action_deleteattachment()
    {
        ...
        if (method_exists($this->bean, 'deleteAttachment')) {
            $duplicate = "false";
            if (isset($_REQUEST['isDuplicate']) && "true" == $_REQUEST['isDuplicate']) {
                $duplicate = "true";
            }
            if (isset($_REQUEST['duplicateSave']) && "true" == $_REQUEST['duplicateSave']) {
                $duplicate = "true";
            }
