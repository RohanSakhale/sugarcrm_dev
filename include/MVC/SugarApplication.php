...
class SugarApplication
{
    ...
    public function loadUser()
    {
        ...
        } elseif (!('Users' == $this->controller->module && in_array($this->controller->action, $allowed_actions))) {
            session_destroy();
            SugarApplication::redirect('index.php?action=Login&module=Users');
            die();
        }
    ...
    public function preProcess()
    {
        ...
        if (!empty($_SESSION['authenticated_user_id'])) {
            if (isset($_SESSION['hasExpiredPassword']) && '1' == $_SESSION['hasExpiredPassword']) {
                ...
                if (empty($ut)
                    && 'AdminWizard' != $this->controller->action && 'EmailUIAjax' != $this->controller->action && 'Wizard' != $this->controller->action && 'SaveAdminWizard' != $this->controller->action && 'SaveUserWizard' != $this->controller->action && 'SaveTimezone' != $this->controller->action && 'Logout' != $this->controller->action) {
                    $this->controller->module = 'Users';
                    $this->controller->action = 'SetTimezone';
                    $record = $GLOBALS['current_user']->id;
                } else {
                    if ('AdminWizard' != $this->controller->action && 'EmailUIAjax' != $this->controller->action && 'Wizard' != $this->controller->action && 'SaveAdminWizard' != $this->controller->action && 'SaveUserWizard' != $this->controller->action) {
                        $this->handleOfflineClient();
                    }
                }
            }
    ...
    public function handleAccessControl()
    {
        ...
        if (!is_admin($GLOBALS['current_user']) && !empty($GLOBALS['adminOnlyList'][$this->controller->module])
            ...
            && (empty($GLOBALS['adminOnlyList'][$this->controller->module][$this->controller->action]) || 'allow' != $GLOBALS['adminOnlyList'][$this->controller->module][$this->controller->action])) {
    ...
    public function checkDatabaseVersion($dieOnFailure = true)
    {
        ...
        if (0 == $row_count && empty($GLOBALS['sugar_config']['disc_client'])) {
            if ($dieOnFailure) {
                $replacementStrings = [
     ...
     * @var array
     */
    protected $whiteListActions = ['index', 'ListView', 'DetailView', 'EditView', 'oauth', 'authorize', 'Authenticate', 'Login', 'SupportPortal', 'GoogleOauth2Redirect'];
