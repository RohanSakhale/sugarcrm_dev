...
class ViewFactory
{
     ...
     * @return valid view
     */
    public static function loadView($type = 'default', $module, $bean = null, $view_object_map = [], $target_module = '')
    {
    ...
    public static function _loadConfig(&$view, $type)
    {
        ...
        if (!empty($view_config['req_params'])) {
            ...
            foreach ($view_config['req_params'] as $key => $value) {
                if (!empty($_REQUEST[$key]) && "false" == $_REQUEST[$key]) {
                    $_REQUEST[$key] = false;
                }
        ...
        if (null == $config && !empty($view_config['actions']) && !empty($view_config['actions'][$action])) {
     ...
     */
    private static function _buildFromFile($file, &$bean, $view_object_map, $type, $module)
    {
        require_once $file;
