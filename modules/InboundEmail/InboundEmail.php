...
class InboundEmail extends SugarBean
{
     ...
     * Sets cache values
     */
    public function setCacheValue($mbox, $insert, $update = [], $remove = [])
    {
        if (empty($mbox)) {
            return;
        }
            ...
            foreach ($insert as $overview) {
                ...
                foreach ($this->overview->fieldDefs as $colDef) {
                    ...
                    if (isset($colDef['len']) && !empty($colDef['len']) &&
                        isset($colDef['type']) && !empty($colDef['type']) && 'varchar' == $colDef['type']) {
    ...
    public function getPop3NewMessagesToDownloadForCron()
    {
        ...
        if (null != $this->groupfolder_id && "" != $this->groupfolder_id && $this->isPop3Protocol()) {
    ...
    public function pop3_checkPartialEmail($synch = false)
    {
        ...
        if (count($diff) > 0) {
            ...
            $this->updateOverviewCacheFile($fetchedOverviews);
            $GLOBALS['log']->info("[EMAIL] Start updating overview cache for pop3 mailbox [{$this->mailbox}] for user [{$current_user->user_name}]");
            return ['status' => "In Progress", 'mbox' => $this->mailbox, 'count' => (count($results) + $startingNo), 'totalcount' => count($diff), 'ieid' => $this->id];
        } // if
...
//            _pp($UIDLs);_pp($cacheUIDLs);
    ...
    public function checkEmailOneMailboxPartial($mailbox, $prefetch = true, $synchronize = false, $start = 0, $max = -1)
    {
        ...
        if (!empty($searchResults)) {
            ...
            $ret = ['status' => $status, 'count' => $start + sizeof($searchResults), 'mbox' => $mailbox, 'totalcount' => $total];
    ...
    public function checkEmailIMAPPartial($prefetch = true, $synch = false)
    {
        ...
        while ('done' == $ret['status'] && $index < sizeof($mailboxes)) {
            if ($ret['count'] > 100) {
    ...
    public function savePersonalEmailAccount($userId = '', $userName = '', $forceSave = true)
    {
        ...
        if (isset($_REQUEST['ssl']) && 1 == $_REQUEST['ssl']) {$useSsl = true;} else {
            $useSsl = false;
        }
        ...
        if (isset($opts['serial']) && !empty($opts['serial'])) {
            ...
            if (isset($_REQUEST['mark_read']) && 1 == $_REQUEST['mark_read']) {
                $this->delete_seen = 0;
            } else {
                $this->delete_seen = 1;
            }
            ...
            if (isset($_REQUEST['only_since']) && 1 == $_REQUEST['only_since']) {
                $onlySince = true;
            } else {
                $onlySince = false;
            }
    ...
    public function findOptimumSettings($useSsl = false, $user = '', $pass = '', $server = '', $port = '', $prot = '', $mailbox = '')
    {
        ...
        if (isset($_REQUEST['ssl']) && 1 == $_REQUEST['ssl']) {
            $useSsl = true;
        }
        ...
        foreach ($serviceArr as $k => $serviceTest) {
            ...
            if (($errors = imap_last_error()) || ($alerts = imap_alerts())) {
                ...
                if ('Too many login failures' == $errors || '[CLOSED] IMAP connection broken (server response)' == $errors // @link http://tools.ietf.org/html/rfc5530#section-3
    ...
    {
        return ('createcase' == $this->mailbox_type && !empty($this->groupfolder_id));
    }
    ...
    public function buildBreadCrumbs($parts, $subtype, $breadcrumb = '0')
    {
        ...
        foreach ($parts as $k => $part) {
            ...
            if (1 == $part->type && (strtoupper($part->subtype) == 'RELATED' || strtoupper($part->subtype) == 'ALTERNATIVE' || strtoupper($part->subtype) == 'MIXED')) {
    ...
    public function handleEncodedFilename($name)
    {
         ...
         *******************************/
        if ('default' != $imapDecode[0]->charset) {
    ...
    public function saveAttachments($msgNo, $parts, $emailId, $breadcrumb = '0', $forDisplay)
    {
        ...
        foreach ($parts as $k => $part) {
            ...
            if (isset($part->parts) && !empty($part->parts) && !(isset($part->subtype) && strtolower($part->subtype) == 'rfc822')) {
                ...
                if (strtolower($part->disposition) == 'attachment' || ((strtolower($part->disposition) == 'inline') && 0 != $part->type)) {
...
//if name is empty, default to filename
            ...
            elseif (2 == $part->type && isset($part->subtype) && strtolower($part->subtype) == 'rfc822') {
            ...
            } elseif (!$part->ifdisposition && 1 != $part->type && 2 != $part->type && '1' != $thisBc) {
    ...
    public function handleTranserEncoding($str, $enc = 0)
    {
        switch ($enc) {
            ...
            // 7BIT or 8BIT
            case 1:
            // already in a string-useable format - do nothing
            case 5:
            // OTHER
            default: // catch all
                $ret = $str;
    ...
    public function importOneEmail($msgNo, $uid, $forDisplay = false, $clean_email = true)
    {
        ...
        if ($forDisplay || $dupeCheckResult) {
            ...
            if (1 == $structure->type && !empty($structure->parts)) {
        ...
        if (!$forDisplay) {
            ...
            if (1 == $this->delete_seen && !$forDisplay) {
...
// POP3 does not support Unseen flags
    ...
    public function connectMailserver($test = false, $force = false)
    {
        ...
        $useSsl = ('true' == $_REQUEST['ssl']) ? true : false;
        if ($test) {
        ...
        if ($test) {
            if (false == $opts && !is_resource($this->conn)) {
                $this->conn = $this->getImapConnection($connectString, $this->email_user, $this->email_password, CL_EXPUNGE);
            }
            $errors = '';
            $alerts = '';
    ...
    public function get_list_view_data()
    {
        ...
        $temp_array['STATUS'] = ('Active' == $this->status) ? $mod_strings['LBL_STATUS_ACTIVE'] : $mod_strings['LBL_STATUS_INACTIVE'];
        return $temp_array;
    }
    ...
    public function moveEmails($fromIe, $fromFolder, $toIe, $toFolder, $uids, $copy = false)
    {
        ...
        } elseif ('folder' == $toIe && 'sugar::Emails' == $fromFolder) {
                ...
                foreach ($exUids as $k => $uid) {
                    ...
                    if (!empty($msgNo)) {
                        ...
                        if ($importStatus) {
                            ...
                            if (!$copy && isset($_REQUEST['delete']) && ("true" == $_REQUEST['delete']) && $importStatus) {
...
eoq;
        ...
        if ("sugar::Emails" == $mbox) {
            ...
            while ($a = $this->db->fetchByAssoc($r)) {
                ...
                $lbl = (0 == $i) ? $app_strings['LBL_EMAIL_ATTACHMENTS'] . ":" : '';
                $i++;
                $attachments .= <<<EOQ
				<tr>
							<td NOWRAP valign="top" class="displayEmailLabel">
...
eoq;
        ...
        // meta-metadata
        $meta['is_sugarEmail'] = ('sugar' == $exMbox[0]) ? true : false;
...
//            _pp("count keys in DESC: ".count($keys));
     ...
     */
    public function displayFolderContents($mbox, $forceRefresh = 'false', $page)
    {
        global $current_user;
        ...
        $ret = [];
        $cacheUsed = false;
        if ('false' == $forceRefresh && $this->validCacheExists($this->mailbox)) {
            $emailSettings = $current_user->getPreference('emailSettings', 'Emails');
    ...
    public function createUserSubscriptionsForGroupAccount()
    {
        ...
        foreach ($usersList as $userObject) {
            ...
            if (false === $previousSubscriptions) {
    ...
    public function createAutoImportSugarFolder()
    {
        ...
        $addSubscriptions = ('Inactive' == $this->status || 'bounce' == $this->mailbox_type) ? false : true;
    ...
    public function displayFetchedSortedListXML($ret, $mbox)
    {
        ...
        foreach ($ret['retArr'] as $msg) {
            $flagged = (0 == $msg->flagged) ? "" : $this->iconFlagged;
            $status = ($msg->deleted) ? $this->iconDeleted : "";
            $status = (0 == $msg->draft) ? $status : $this->iconDraft;
            $status = (0 == $msg->answered) ? $status : $this->iconAnswered;
