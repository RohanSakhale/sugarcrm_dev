...
EOQ;
    ...
    public function verify_data($ieVerified = true)
    {
        ...
        if (is_admin($current_user)) {
            ...
            if (($remaining_admins <= 1) && ('1' != $this->is_admin) && ($this->id == $current_user->id)) {
    ...
    public function isAdmin()
    {
        if (isset($this->is_admin)
            && ('1' == $this->is_admin || 'on' === $this->is_admin)) {
            return true;
        }
        return false;
    }
    ...
    public function sendEmailForPassword($templateId, array $additionalData = [])
    {
        ...
        if ('smtp' == $mail->Mailer && '' == $mail->Host && $current_user->is_admin) {
        ...
        if (true == $result['status']) {
            ...
            $emailObj->save();
            if (!isset($additionalData['link']) || false == $additionalData['link']) {
    ...
    {
        if (false == $this->user_hash && !$this->is_group
            && !$this->portal_only
