...
class ACLAction extends SugarBean
{
     ...
     */
    public static function hasAccess($is_owner = false, $access = 0)
    {

        if (0 != $access && ACL_ALLOW_ALL == $access || ($is_owner && ACL_ALLOW_OWNER == $access)) {
            return true;
        }

        //if this exists, then this function is not static, so check the aclaccess parameter
        if (isset($this) && isset($this->aclaccess)) {
            if (ACL_ALLOW_ALL == $this->aclaccess || ($is_owner && ACL_ALLOW_OWNER == $this->aclaccess)) {
                return true;
            }
        }
        return false;
    ...
    public static function userNeedsOwnership($user_id, $category, $action, $type = 'module')
    {
        ...
        if (!empty($_SESSION['ACL'][$user_id][$category][$type][$action])) {
            return ACL_ALLOW_OWNER == $_SESSION['ACL'][$user_id][$category][$type][$action]['aclaccess'];
        }
        return false;
    }
        ...
        foreach ($categories as $cat_name => $category) {
            foreach ($category as $type_name => $type) {
                foreach ($type as $act_name => $action) {
                    ...
                    if ('module' == $type_name) {
                        if ('aclaccess' != $act_name && ACL_ALLOW_DISABLED == $categories[$cat_name]['module']['access']['aclaccess']) {
                    ...
                    if ('Users' == $cat_name && 'admin' == $act_name) {
