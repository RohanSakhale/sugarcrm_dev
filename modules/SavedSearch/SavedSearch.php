...
class SavedSearch extends SugarBean
{
    ...
    // Saved Search Form
    public function getForm($module, $inline = true)
    {
        ...
        if (isset($_REQUEST['saved_search_select']) && '_none' != $_REQUEST['saved_search_select']) {
            $this->retrieveSavedSearch($_REQUEST['saved_search_select']);
        }

        if ((!empty($_REQUEST['displayColumns']) && 'undefined' != $_REQUEST['displayColumns']) || (isset($this->contents['displayColumns']) && 'undefined' != $this->contents['displayColumns'])) {
            // columns to display
            if (!empty($_REQUEST['displayColumns']) && 'undefined' != $_REQUEST['displayColumns']) {
    ...
    public function getSelect($module)
    {
        ...
        if (!empty($_SESSION['LastSavedView'][$module]) && (('ListView' == $_REQUEST['action']) || ('index' == $_REQUEST['action']))) {
    ...
    public function handleSave($prefix, $redirect = true, $useRequired = false, $id = null, $searchModuleBean)
    {
        ...
        foreach ($contents as $input => $value) {
            ...
            if (!empty($value) && preg_match('/^(start_range_|end_range_|range_)?(.*?)(_advanced|_basic)$/', $input, $match)) {
                ...
                if (isset($searchModuleBean->field_defs[$field]['type']) && empty($searchModuleBean->field_defs[$field]['disable_num_format'])) {
                    ...
                    if (('date' == $type || 'datetime' == $type || 'datetimecombo' == $type) && !preg_match('/^\[.*?\]$/', $value)) {
                        $db_format = $timedate->to_db_date($value, false);
                        $contents[$input] = $db_format;
                    } else if ('int' == $type || 'currency' == $type || 'decimal' == $type || 'float' == $type) {
                        if (preg_match('/[^\d]/', $value)) {
                            ...
                            if ('currency' == $type && preg_match('/^([^\d])/', $value, $match)) {
    ...
    public function populateRequest()
    {
        ...
        foreach ($this->contents as $key => $val) {
            if ('advanced' != $key && 'module' != $key && !strpos($key, '_ORDER_BY') && 'lvso' != $key) {
                if (isset($searchModuleBean) && !empty($val) && preg_match('/^(start_range_|end_range_|range_)?(.*?)(_advanced|_basic)$/', $key, $match)) {
                    ...
                    if (isset($searchModuleBean->field_defs[$field]['type']) && empty($searchModuleBean->field_defs[$field]['disable_num_format'])) {
                        ...
                        //Avoid macro values for the date types
                        if (('date' == $type || 'datetime' == $type || 'datetimecombo' == $type) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $val) && !preg_match('/^\[.*?\]$/', $val)) {
                            $val = $timedate->to_display_date($val, false);
                        } else if (('int' == $type || 'currency' == $type || 'decimal' == $type || 'float' == $type) && isset($this->contents[$key . '_unformatted_number']) && preg_match('/^\d+$/', $val)) {
                            require_once 'modules/Currencies/Currency.php';
                            $val = format_number($val);
                            if ('currency' == $type && isset($this->contents[$key . '_currency_symbol'])) {
