...
class Lead extends Person
{
    ...
    public $relationship_fields = ['email_id' => 'emails', 'call_id' => 'calls', 'meeting_id' => 'meetings', 'task_id' => 'tasks'];
    ...
    public function get_linked_fields()
    {
        ...
        if (!empty($fieldDefs)) {
            foreach ($fieldDefs as $name => $properties) {
                if ('oldmeetings' == $name || 'oldcalls' == $name) {continue;} elseif (array_search('link', $properties) === 'type') {
...
//carrys forward custom lead fields to contacts, accounts, opportunities during Lead Conversion
    public function convertCustomFieldsForm(&$form, &$tempBean, &$prefix)
    {
        ...
        foreach ($this->field_defs as $field => $value) {
            if (!empty($value['source']) && 'custom_fields' == $value['source']) {
                if (!empty($tempBean->field_defs[$field]) and isset($tempBean->field_defs[$field])) {
                    ...
                    if (!empty($tempBean->custom_fields->avail_fields[$field]['required']) and ((1 == $tempBean->custom_fields->avail_fields[$field]['required']) or ('1' == $tempBean->custom_fields->avail_fields[$field]['required']) or ('true' == $tempBean->custom_fields->avail_fields[$field]['required']) or (true == $tempBean->custom_fields->avail_fields[$field]['required']))) {
                        $form .= "&nbsp;<span class='required'>" . $lbl_required_symbol . "</span>";
                    }
                    $form .= "</td></tr>";
                    $form .= "<tr><td nowrap colspan='4' class='dataField' nowrap>";

                    if (isset($value['isMultiSelect']) && 1 == $value['isMultiSelect']) {
                    ...
                    if (!empty($value['options']) and isset($value['options'])) {
                        ...
                        $form .= "</select";
                    } elseif ('bool' == $value['type']) {
                        if ((1 == $this->$field) or ('1' == $this->$field)) {$checked = 'checked';} else { $checked = '';}
