...
class Call extends SugarBean
{
    ...
    public function save($check_notify = false)
    {
        ...
        if (!empty($_REQUEST['send_invites']) && '1' == $_REQUEST['send_invites']) {
            $check_notify = true;
        } else {
            $check_notify = false;
        }
        if (empty($_REQUEST['send_invites'])) {
            ...
            if ((empty($this->id) && isset($_REQUEST['assigned_user_id']) && !empty($_REQUEST['assigned_user_id']) && $GLOBALS['current_user']->id != $_REQUEST['assigned_user_id']) || (isset($old_assigned_user_id) && !empty($old_assigned_user_id) && isset($_REQUEST['assigned_user_id']) && !empty($_REQUEST['assigned_user_id']) && $old_assigned_user_id != $_REQUEST['assigned_user_id'])) {
                ...
                if (!isset($GLOBALS['resavingRelatedBeans']) || false == $GLOBALS['resavingRelatedBeans']) {
                    $check_notify = true;
                }
    ...
    public function fill_in_additional_detail_fields()
    {
        ...
        if (isset($_REQUEST['parent_type']) && (!isset($_REQUEST['action']) || 'SubpanelEdits' != $_REQUEST['action'])) {
    ...
    public function get_list_view_data()
    {
        ...
        if (isset($this->parent_type) && null != $this->parent_type) {
        ...
        $call_fields['REMINDER_CHECKED'] = -1 == $this->reminder_time ? false : true;
        $call_fields['EMAIL_REMINDER_CHECKED'] = -1 == $this->email_reminder_time ? false : true;
    ...
    public function set_notification_body($xtpl, $call)
    {
        ...
        $notifyUser = ('User' == $call->current_notify_user->object_name) ? $call->current_notify_user : $current_user;
