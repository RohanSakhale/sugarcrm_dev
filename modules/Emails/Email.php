...
class Email extends SugarBean
{
    ...
    {
        return (isset($request['saveDraft']) || ('draft' == $this->type && 'draft' == $this->status));
    }
    ...
    public function email2Send($request)
    {
        ...
        if ($this->isDraftEmail($request)) {
            if ('draft' != $this->type && 'draft' != $this->status) {
                $this->id = create_guid();
                $this->new_with_id = true;
                $this->date_entered = "";
            } // if
        ...
        if (isset($request['saveDraft'])) {
            ...
            if (isset($_REQUEST['parent_type']) && !empty($_REQUEST['parent_type']) &&
                ...
                ('Accounts' == $_REQUEST['parent_type'] || 'Contacts' == $_REQUEST['parent_type'] || 'Leads' == $_REQUEST['parent_type'] || 'Users' == $_REQUEST['parent_type'] || 'Prospects' == $_REQUEST['parent_type'])) {
        ...
        if (!empty($request['attachments'])) {
            ...
            foreach ($exAttachments as $file) {
                ...
                if (!empty($file)) {
                    ...
                    if ((('draft' == $this->type) && !empty($this->id)) || (isset($request['saveToSugar']) && 1 == $request['saveToSugar'])) {
                        $note = new Note();
                        $note->id = create_guid();
            ...
            foreach ($exNotes as $noteId) {
                ...
                if (!empty($noteId)) {
                    ...
                    if (!empty($note->id)) {
                        ...
                        if ((('draft' == $this->type) && !empty($this->id))
                            || (isset($request['saveToSugar']) && 1 == $request['saveToSugar'])) {
        ...
        if ('draft' == $this->type && !isset($request['saveDraft'])) {
        ...
        if ((!(empty($orignialId) || isset($request['saveDraft']) || ('draft' == $this->type && 'draft' == $this->status))) &&
            (('reply' == $_REQUEST['composeType']) || ('replyAll' == $_REQUEST['composeType']) || ('replyCase' == $_REQUEST['composeType'])) && ($orignialId != $this->id)) {
        ...
        if ('reply' == $_REQUEST['composeType'] || 'replyCase' == $_REQUEST['composeType']) {
        ...
        if (!empty($request['fromAccount'])) {
            if (isset($ie->id) && !$ie->isPop3Protocol() && 'gmail' != $mail->oe->mail_smtptype) {
    ...
    ///////////////////////////////////////////////////////////////////////////
    ////    RETRIEVERS
    public function retrieve($id, $encoded = true, $deleted = true)
    {
    ...
    public function check_email_settings()
    {
        ...
        if (!empty($send_type) && "SMTP" == $send_type) {
    ...
    public function handleAttachments()
    {
        ...
        if (('out' == $this->type || 'draft' == $this->type) && 'draft' == $this->status && isset($_REQUEST['record'])) {
        ...
        if (('out' == $this->type || 'draft' == $this->type) &&
            isset($_REQUEST['origType']) && 'forward' == $_REQUEST['origType'] &&
        ...
        if (isset($_REQUEST['prior_attachments']) && !empty($_REQUEST['prior_attachments']) && true == $this->new_with_id) {
        ...
        for ($i = 0; $i < $max_files_upload; $i++) {
            ...
            if (0 != $_FILES['email_attachment' . $i]['error'] && 4 != $_FILES['email_attachment' . $i]['error']) {
    ...
    public function handleBody($mail)
    {
        ...
        if ((isset($_REQUEST['setEditor']) /* from Email EditView navigation */
            && 1 == $_REQUEST['setEditor'] && trim($_REQUEST['description_html']) != '')
        ...
        foreach (explode("\n", ("text/html" == $mail->ContentType) ? $mail->AltBody : $mail->Body) as $line) {
            if (strlen($line) > 998) {
    ...
    public function send()
    {
        ...
        foreach ($this->saved_attachments as $note) {
            ...
            } elseif ('DocumentRevision' == $note->object_name) {
                // from Documents
    ...
    public function create_new_list_query($order_by, $where, $filter = [], $params = [], $show_deleted = 0, $join_type = '', $return_array = false, $parentbean = null, $singleSelect = false)
    {
    ...
    public function fill_in_additional_list_fields()
    {
        ...
        ///////////////////////////////////////////////////////////////////////
        if (empty($this->contact_id) && !empty($this->parent_id) && !empty($this->parent_type) && 'Contacts' === $this->parent_type && !empty($this->parent_name)) {
    ...
    public function get_list_view_data()
    {
        ...
        if ('replied' != $this->status) {
            ...
            $email_fields['STATUS'] = (1 == $email_fields['REPLY_TO_STATUS'] ? $mod_strings['LBL_REPLIED'] : $email_fields['STATUS']);
        } else {
    ...
    public function searchImportedEmails($sort = '', $direction = '')
    {
        ...
        $total_count = (null != $count_row) ? $count_row['c'] : 0;
        ...
        while ($a = $this->db->fetchByAssoc($rs)) {
            $temp = [];
            $temp['flagged'] = (is_null($a['flagged']) || '0' == $a['flagged']) ? '' : 1;
            $temp['status'] = (is_null($a['reply_to_status']) || '0' == $a['reply_to_status']) ? '' : 1;
            $temp['subject'] = $a['name'];
            $temp['date'] = $timedate->to_display_date_time($a['date_sent']);
            $temp['uid'] = $a['id'];
            $temp['ieId'] = $a['mailbox_id'];
            $temp['site_url'] = $sugar_config['site_url'];
            $temp['seen'] = ('unread' == $a['status']) ? 0 : 1;
            $temp['type'] = $a['type'];
    ...
    public function _genereateSearchImportedEmailsQuery()
    {
        ...
        $addDistinctKeyword = "";
        if (!empty($_REQUEST['attachmentsSearch']) && 1 == $_REQUEST['attachmentsSearch']) //1 indicates yes
        {
            $query['where'] .= " AND EXISTS ( SELECT id FROM notes n WHERE n.parent_id = emails.id AND n.deleted = 0 AND n.filename is not null )";
        } else if (!empty($_REQUEST['attachmentsSearch']) && 2 == $_REQUEST['attachmentsSearch']) {
        ...
        if (isset($this->field_defs[$field]) && is_array($this->field_defs[$field])) {
            if (empty($this->modifiedFieldDefs[$field])) {
                if (
                    ...
                    (strtolower($this->field_defs[$field]['isnull']) == 'false' || false === $this->field_defs[$field]['isnull'])
                ) {
                    $this->modifiedFieldDefs[$field]['isnull'] = $this->field_defs[$field]['isnull'];
                    unset($this->field_defs[$field]['isnull']);
                }

                if (isset($this->field_defs[$field]['dbType']) && 'id' == $this->field_defs[$field]['dbType']) {
