...
class Meeting extends SugarBean
{
    ...
    public $relationship_fields = ['account_id' => 'accounts', 'opportunity_id' => 'opportunity', 'case_id' => 'case',
...
//        $this->fill_in_additional_detail_fields();
    ...
    public function ACLAccess($view, $is_owner = 'not_set')
    {
        ...
        if (false == $this->syncing) {
            ...
            switch ($view) {
                ...
                case 'editview':
                case 'delete':
                    if (!empty($this->recurring_source) && "Sugar" != $this->recurring_source) {
                        return false;
                    }
            }
        }
    ...
    public function save($check_notify = false)
    {
        ...
        $check_notify = (!empty($_REQUEST['send_invites']) && '1' == $_REQUEST['send_invites']) ? true : false;
        ...
        if (empty($this->id) && !empty($_REQUEST['module']) && "Calendar" == $_REQUEST['module'] && !empty($_REQUEST['repeat_type']) && !empty($this->repeat_parent_id)) {
            $check_notify = false;
        }
        ...
        if (!empty($this->type) && 'Sugar' != $this->type) {
    ...
    public function fill_in_additional_detail_fields()
    {
        ...
        $this->reminder_checked = -1 == $this->reminder_time ? false : true;
        ...
        $this->email_reminder_checked = -1 == $this->email_reminder_time ? false : true;
    ...
    public function get_list_view_data()
    {
        ...
        $meeting_fields['REMINDER_CHECKED'] = -1 == $this->reminder_time ? false : true;
        $meeting_fields['EMAIL_REMINDER_CHECKED'] = -1 == $this->email_reminder_time ? false : true;
    ...
    public function set_notification_body($xtpl, &$meeting)
    {
        ...
        $notifyUser = ('User' == $meeting->current_notify_user->object_name) ? $meeting->current_notify_user : $current_user;
    ...
    public function set_accept_status(&$user, $status)
    {
        ...
        } else if ('Contact' == $user->object_name) {
...
//if a meeting is being imported then contact_id  should not be excluded
...
function getMeetingsExternalApiDropDown($focus = null, $name = null, $value = null, $view = null)
{
    ...
    if ("eapm_list" != $dictionary['Meeting']['fields']['type']['options']) {
