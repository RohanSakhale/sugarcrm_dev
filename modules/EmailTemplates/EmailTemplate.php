        ...
        foreach ($loopControl as $collectionKey => $beans) {
            ...
            foreach ($beans as $beankey => $bean) {
                foreach ($bean->field_defs as $key => $field_def) {
                    if (('relate' == $field_def['type'] && empty($field_def['custom_type'])) ||
                        ('assigned_user_name' == $field_def['type'] || 'link' == $field_def['type']) ||
                        ('bool' == $field_def['type']) ||
...
//function all string that match the pattern {.} , also catches the list of found strings.
                    ...
                    if ($count > 0) {
                        for ($i = ($count - 1); $i >= 0; $i--) {
                            ...
                            if (!empty($tracked_urls[$url_key_name])) {
                                if (1 == $tracked_urls[$url_key_name]['is_optout']) {
                ...
                if (0 != $count) {
                    for ($i = ($count - 1); $i >= 0; $i--) {
                        if (!isset($matches[0][$i][2])) {
                            ...
                            if (isset($focus->field_defs[$matches[0][$i][2]]) && 'enum' == $focus->field_defs[$matches[0][$i][2]]['type'] && isset($focus->field_defs[$matches[0][$i][2]]['options'])) {
    ...
    public function _parseUserValues($repl_arr, &$user)
    {
        foreach ($user->field_defs as $field_def) {
            if (('relate' == $field_def['type'] && empty($field_def['custom_type'])) || 'assigned_user_name' == $field_def['type']) {
                continue;
            }

            if ('enum' == $field_def['type']) {
    ...
    public function parse_template_bean($string, $bean_name, &$focus)
    {
        ...
        if ('Contacts' == $bean_name) {
            ...
            foreach ($contact->field_defs as $field_def) {
                if (('relate' == $field_def['type'] && empty($field_def['custom_type'])) || 'assigned_user_name' == $field_def['type'] || 'link' == $field_def['type']) {
                    continue;
                }
        ...
        foreach ($focus->field_defs as $field_def) {
            if (isset($focus->$field_def['name'])) {
                ...
                if ('enum' == $field_def['type'] && isset($field_def['options'])) {
    ...
    public function parse_template($string, &$bean_arr)
    {
        ...
        foreach ($bean_arr as $bean_name => $bean_id) {
            ...
            if ('Leads' == $bean_name || 'Prospects' == $bean_name) {
                $bean_name = 'Contacts';
            }

            if (isset($this) && isset($this->module_dir) && 'EmailTemplates' == $this->module_dir) {
