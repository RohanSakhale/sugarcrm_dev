...
class Scheduler extends SugarBean
{
    ...
    public static function initUser()
    {
        ...
        if (false === $adminId) {
// Retrieve another admin if default admin doesn't exist
            $adminId = $db->getOne(
...
// Scheduler jobs run as default Admin
    ...
    {
        if (empty($this->id)) {
            // execute only if we have an instance
    ...
    public function deriveDBDateTimes($focus)
    {
        ...
        if ('*' == $mons) {
            ...
            $monName = [];
            if (strstr($mons, ',')) {
                // we have particular (groups) of months
                $exMons = explode(',', $mons);
        ...
        if (!empty($focus->date_time_end)) {
            // do the same for date_time_end if not empty
...
//        _pp('currentHour: '. $currentHour);
        ...
        foreach ($hrName as $kHr => $hr) {
            foreach ($minName as $kMin => $min) {
                ...
                if ($tsGmt >= $timeStartTs) {
                    // start is greater than the date specified by admin
    ...
    public function handleIntervalType($type, $value, $mins, $hours)
    {
        ...
        switch ($type) {
            ...
            case 1:
                ...
                global $current_user;
                if (preg_match('/\*\//', $value)) {
                    // every [SOME INTERVAL] hours
                    $value = str_replace('*/', '', $value);
                    return $value . $mod_strings['LBL_HOUR'];
                } elseif (preg_match('/[^0-9]/', $mins)) {
...
//                    $error = '<em>'.$mod_strings['LBL_NO_PHP_CLI'].'</em>';
    ...
    public function get_list_view_data()
    {
        ...
        if ('2020-12-31 23:59' == $this->date_time_end || '' == $this->date_time_end) {
