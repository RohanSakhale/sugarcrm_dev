...
class DynamicField
{
    ...
    public function buildCache($module = false, $saveCache = true)
    {
        ...
        if (!empty($GLOBALS['installing']) && true == $GLOBALS['installing'] || empty($GLOBALS['db'])) {
            return false;
        }
    ...
    public function getJOIN($expandedList = false, $includeRelates = false, &$where = false)
    {
        ...
        if (empty($expandedList)) {
            ...
            foreach ($this->bean->field_defs as $name => $field) {
                if (!empty($field['source']) && 'custom_fields' == $field['source'] && (!$isList || !empty($expandedList[$name]))) {
                    ...
                    if ('html' != $field['type'] && 'parent_name' != $name) {
        ...
        if ($includeRelates) {
            ...
            $jtCount = 1;
            foreach ($this->bean->field_defs as $name => $field) {
                if ('relate' == $field['type'] && isset($field['custom_module'])) {
    ...
    {
        if (empty($field_def['type']) || "relate" != $field_def['type']) {
            return false;
        }
        ...
        if (isset($rel_mod->field_defs['name'])) {
            ...
            else if (!empty($rel_mod->field_defs['name']['source']) && "non-db" == $rel_mod->field_defs['name']['source'] && !empty($field_def['rname'])) {
    ...
    public function save($isUpdate)
    {
        ...
        if ($this->bean->hasCustomFields() && isset($this->bean->id)) {
            ...
            foreach ($this->bean->field_defs as $name => $field) {
                ...
                if ('html' == $field['type'] || 'parent' == $field['type']) {
                    continue;
                }
                ...
                if (isset($this->bean->$name)) {
                    ...
                    if (in_array($field['type'], ['int', 'float', 'double', 'uint', 'ulong', 'long', 'short', 'tinyint', 'currency', 'decimal'])) {
                        $quote = '';
                    ...
                    if ('bool' == $field['type']) {
                        if (false === $this->bean->$name) {
                            $this->bean->$name = '0';
                        } elseif (true === $this->bean->$name) {
                            $this->bean->$name = '1';
                        }
                    }

                    $val = $this->bean->$name;
                    if (('date' == $field['type'] || 'datetimecombo' == $field['type']) && (empty($this->bean->$name) || '1900-01-01' == $this->bean->$name)) {
                        $quote = '';
                        $val = 'NULL';
    ...
    public function deleteField($widget)
    {
        ...
        if (empty($GLOBALS['dictionary'][$object_name])) {
            ...
            $object_name = false != $newName ? $newName : $object_name;
        }
    ...
    public function saveExtendedAttributes($field, $column_fields)
    {
        ...
        foreach ($field->vardef_map as $property => $fmd_col) {
            ...
            if (!isset($field->$property) || in_array($fmd_col, $column_fields) || in_array($property, $column_fields)
                ...
                || "action" == $property || "label_value" == $property || "label" == $property || (substr($property, 0, 3) == 'ext' && strlen($property) == 4)) {
                continue;
            }
            $to_save[$property] =
    ...
    protected function isDefaultValue($property, $value, $baseField)
    {
        switch ($property) {
            case "importable":
                return ('true' === $value || '1' === $value || true === $value || 1 === $value);
                break;
            case "required":
            case "audited":
            case "massupdate":
                return ('false' === $value || '0' === $value || false === $value || 0 === $value);
                break;
            case "default_value":
            case "default":
            case "help":
            ...
            case "duplicate_merge":
                return ('false' === $value || '0' === $value || false === $value || 0 === $value || "disabled" === $value);
                break;
        }
    ...
    protected function writeVardefExtension($bean_name, $field, $def_override)
    {
        ...
        $vBean = "aCase" == $bean_name ? "Case" : $bean_name;
    ...
    public function addField($name, $label = '', $type = 'Text', $max_size = '255', $required_option = 'optional', $default_value = '', $ext1 = '', $ext2 = '', $ext3 = '', $audited = 0, $mass_update = 0, $ext4 = '', $help = '', $duplicate_merge = 0, $comment = '')
    {
        ...
        $field->type = $type;
        $field->len = $max_size;
        $field->required = (!empty($required_option) && 'optional' != $required_option);
    ...
    public function add_existing_custom_fields($execute = true)
    {
        ...
        if ($this->bean->hasCustomFields()) {
            foreach ($this->bean->field_defs as $name => $data) {
                if (empty($data['source']) || 'custom_fields' != $data['source']) {
                    continue;
                }
    ...
    public function getDBName($name, $_C = true)
    {
        ...
        $exclusions = ['parent_type', 'parent_id', 'currency_id', 'parent_name'];
        // Remove any non-db friendly characters
        $return_value = preg_replace("/[^\w]+/", "_", $name);
        if (true == $_C && !in_array($return_value, $exclusions) && substr($return_value, -2) != '_c') {
            $return_value .= '_c';
        }
