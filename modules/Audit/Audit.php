...
class Audit extends SugarBean
{
    ...
    public function get_audit_list()
    {
        ...
        if (!empty($_REQUEST['record'])) {
            ...
            if (null == $result || !$focus->ACLAccess('', $focus->isOwner($current_user->id))) {
            ...
            while (($row = $focus->db->fetchByAssoc($result)) != null) {
                ...
                foreach ($fieldDefs as $field) {
                    if (isset($row[$field['name']])) {
                        if (('before_value_string' == $field['name'] || 'after_value_string' == $field['name']) &&
                        ...
                        if (('before_value_string' == $field['name'] || 'after_value_string' == $field['name']) && ("enum" == $row['data_type'] || "multienum" == $row['data_type'])) {
                            ...
                            if ('date' === $temp_list['data_type']) {
                                $temp_list[$field['name']] = $timedate->to_display_date($temp_list[$field['name']], false);
                            }
                        } elseif (('before_value_string' == $field['name'] || 'after_value_string' == $field['name']) && ("datetimecombo" == $row['data_type'])) {
                            if (!empty($temp_list[$field['name']]) && 'NULL' != $temp_list[$field['name']]) {
                                $temp_list[$field['name']] = $timedate->to_display_date_time($temp_list[$field['name']]);
                            } else {
                                $temp_list[$field['name']] = '';
                            }
                        } elseif ('field_name' == $field['name']) {
                            global $mod_strings;
