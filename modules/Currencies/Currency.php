...
class Currency extends SugarBean
{
    ...
    public function list_view_parse_additional_sections(&$list_form)
    {
        ...
        if (isset($isMerge) && $isMerge && '-99' != $this->id) {
    ...
    public function retrieve($id = -1, $encode = true, $deleted = true)
    {
        ...
        if (!isset($this->name) || 1 == $this->deleted) {
            $this->name = $this->getDefaultCurrencyName();
            $this->symbol = $this->getDefaultCurrencySymbol();
            $this->conversion_rate = 1;
            $this->iso4217 = $this->getDefaultISO4217();
...
function format_number($amount, $round = null, $decimals = null, $params = [])
{
    ...
    if (!empty($params['currency_symbol']) && $params['currency_symbol']) {
        ...
        } elseif (!empty($params['type']) && 'pdf' == $params['type']) {
...
{
    $view = ucfirst($view);
    if ('EditView' == $view || 'MassUpdate' == $view || 'QuickCreate' == $view || 'ConvertLead' == $view) {
...
{
    if ('EditView' == $view || 'MassUpdate' == $view || 'QuickCreate' == $view) {
        require_once 'modules/Currencies/ListCurrency.php';
        $currency_fields = [];
        //Bug 18276 - Fix for php 5.1.6
        $defs = $focus->field_defs;
