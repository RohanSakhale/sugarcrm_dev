...
class Configurator
{
    public $config = '';
    public $override = '';
    public $allow_undefined = ['stack_trace_errors', 'export_delimiter', 'use_real_names', 'developerMode', 'default_module_favicon', 'authenticationClass', 'SAML_loginurl', 'SAML_X509Cert', 'dashlet_auto_refresh_min', 'show_download_tab', 'enable_action_menu'];
    public $errors = ['main' => ''];
    public $logger = null;
    ...
    public function handleOverride($fromParseLoggerSettings = false)
    {
        ...
        if (!$this->useAuthenticationClass && !$fromParseLoggerSettings) {
            if (isset($overrideArray['authenticationClass']) && 'SAMLAuthenticate' == $overrideArray['authenticationClass']) {
    ...
    public function clearCache()
    {
        ...
        foreach ($currentConfigArray as $key => $val) {
            if (in_array($key, $this->allow_undefined) || isset($sugar_config[$key])) {
                if (empty($val)) {
                    if (!empty($this->previous_sugar_override_config_array['stack_trace_errors']) && 'stack_trace_errors' == $key) {
